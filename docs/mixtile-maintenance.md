...
This repository keeps one Mixtile release config per supported stable series:

- `configs/mixtile-rockchip-24.10.config`
- `configs/mixtile-rockchip-25.12.config`

Both configs build only `rockchip/armv8` and enable these profiles:
...
- `/home/mixtile/work/mixtile-openwrt-24.10`
- `/home/mixtile/work/mixtile-openwrt-25.12`
...
`staging_dir`, and `bin/` isolated per branch.

Validation sequence:

1. Check out the target maintenance branch in a clean git directory.
2. Run `./scripts/feeds update -a && ./scripts/feeds install -a`.
3. Copy the matching `configs/mixtile-rockchip-<series>.config` to `.config`.
4. Run `make defconfig`.
5. Run `make download -j12`.
6. Run `make -j12 BUILD_LOG=1`.
7. Confirm `bin/targets/rockchip/armv8/` contains both device images,
